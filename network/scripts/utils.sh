...
ORDERER_CA=/opt/gopath/src/github.com/hyperledger/fabric/peer/crypto/ordererOrganizations/property-registration-network.com/orderers/orderer.pharma-network.com/msp/tlscacerts/tlsca.property-registration-network.com-cert.pem
PEER0_CONSUMER_CA=/opt/gopath/src/github.com/hyperledger/fabric/peer/crypto/peerOrganizations/registrar.property-registration-network.com/peers/peer0.consumer.pharma-network.com/tls/ca.crt
...
PEER0_MANUFACTUERER_CA=/opt/gopath/src/github.com/hyperledger/fabric/peer/crypto/peerOrganizations/manufacturer.pharma-network.com/peers/peer0.manufacturer.pharma-network.com/tls/ca.crt
...
verifyResult() {
  if [ "$1" -ne 0 ]; then
    echo "!!!!!!!!!!!!!!! "$2" !!!!!!!!!!!!!!!!"
    echo "========= ERROR !!! FAILED to execute pharmanet Network Bootstrap ==========="
    echo
    exit 1
  fi
}
...
setGlobals() {
  ...
  if [ "$ORG" == 'consumer' ]; then
    ...
    if [ "$PEER" -eq 0 ]; then
      CORE_PEER_ADDRESS=peer0.distributor.pharma-network.com:13051
    fi
    if [ "$PEER" -eq 1 ]; then
      CORE_PEER_ADDRESS=peer1.distributor.pharma-network.com:14051
    fi
    ...
    if [ "$PEER" -eq 0 ]; then
      CORE_PEER_ADDRESS=peer0.retailer.pharma-network.com:15051
    else
      CORE_PEER_ADDRESS=peer1.retailer.pharma-network.com:16051
    fi
    ...
    if [ "$PEER" -eq 0 ]; then
      CORE_PEER_ADDRESS=peer0.transporter.pharma-network.com:17051
    else
      CORE_PEER_ADDRESS=peer1.transporter.pharma-network.com:18051
    fi
  else
...
upgradeChaincode() {
  ...
  if [ -z "$CORE_PEER_TLS_ENABLED" -o "$CORE_PEER_TLS_ENABLED" = "false" ]; then
    set -x
    peer chaincode upgrade -o orderer.pharma-network.com:7050 -C $CHANNEL_NAME -n pharmanet -l ${LANGUAGE} -v ${VERSION} -p ${CC_SRC_PATH} -c '{"Args":["org.pharma-network.com.pharmanet:instantiate"]}' -P "OR ('manufacturerMSP.member','distributorMSP.member','retailerMSP.member','transporterMSP.member','consumerMSP.member')" >&log.txt
    res=$?
    set +x
  else
    set -x
